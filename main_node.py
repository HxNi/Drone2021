...
class DroneControl:
  def __init__(self):
    ...
    self.tp = [0, 0, 2.1, 0]
    self.wp = [['v', [5.5, 0, 2.1, 0], [1, 0]], ['v', [0, 0, 2.1, 0], [-1, 0]]]
    self.current_wp = 0
    self.br = CvBridge()
  ...
  def init(self):
    ...
    rospy.loginfo("Initialize#!/usr/bin/env python
from cv_bridge import CvBridge
import cv2
import rospy
from math import sqrt
...
d")
  ...
  def log_position(self):
    lp = self.d.getLocalPosition()
    dist = sqrt((self.wp[self.current_wp][1][0] - lp.x)**2 + (self.wp[self.current_wp][1][1] - lp.y)**2 + (self.wp[self.current_wp][1][2] - lp.z)**2)
    s = "%+.2f %+.2f %+.2f D %.2f" % (lp.x, lp.y, lp.z, dist)
    rospy.loginfo(s)
